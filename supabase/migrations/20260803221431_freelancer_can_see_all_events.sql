-- Permissão por membro: ver a agenda de TODOS os eventos da casa (não só os que está escalado).
-- Backfill preserva o comportamento atual (funcionários e quem delega já viam tudo).
ALTER TABLE freelancers ADD COLUMN IF NOT EXISTS can_see_all_events boolean NOT NULL DEFAULT false;
UPDATE freelancers SET can_see_all_events = true
 WHERE can_see_all_events = false AND (staff_type = 'funcionario' OR can_delegate_tasks = true);

-- agenda_for_freelancer passa a usar a flag explícita para liberar "próximos eventos da casa"
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Próximos eventos da casa: só para quem tem permissão explícita

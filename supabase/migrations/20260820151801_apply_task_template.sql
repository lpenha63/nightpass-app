-- Gera as tarefas do modelo para UM evento, entregando cada item a quem está escalado
-- na área correspondente. Sem SECURITY DEFINER de propósito: o RLS do chamador continua
-- valendo, então só um membro da casa consegue gerar (e só na própria casa).
...
DECLARE
  ...
  it record; m record; n_escalados int;
BEGIN
  SELECT e.house_id,
  ...
  FOR it IN SELECT * FROM task_template_items WHERE template_id = p_template ORDER BY sort_order, title LOOP
    n_escalados := 0;
    FOR m IN
      -- mesma regra de área usada na tela: escala, senão a 1ª área do cadastro
    ...
    LOOP
      ...
      -- idempotente: gerar de novo não duplica a mesma tarefa para a mesma pessoa
      ...
      END IF;
    END LOOP;
    -- área sem ninguém escalado: o gestor precisa saber, senão some sem explicação

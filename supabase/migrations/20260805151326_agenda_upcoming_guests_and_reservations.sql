-- Agenda da casa: inclui total de CONVIDADOS (listas + pessoas em reservas)
    ...
    'upcoming', case when coalesce(f.can_see_all_events, false) then coalesce((
      ...
      from (
        select jsonb_build_object(
          ...
          -- Convidados previstos = pessoas em reservas + convidados de listas
          'convidados', (
...
-- Reservas de um evento para a equipe montar as mesas (só quem vê todos os eventos)
...
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      'id', r.id, 'name', r.name, 'people', r.people_count, 'location', r.location,

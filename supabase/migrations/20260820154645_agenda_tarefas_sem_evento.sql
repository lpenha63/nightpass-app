-- BUG: a agenda do app pendurava as tarefas dentro dos eventos. Tarefa sem evento
-- (que é justamente o PADRÃO do modal "Nova tarefa" e todo o "Dia a dia" do gestor)
-- não tinha onde se pendurar e NUNCA chegava ao colaborador — só o WhatsApp avisava.
-- Passa a vir num balde próprio: 'avulsas'.
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Tarefas do dia a dia (sem evento). Concluídas só dos últimos 30 dias, senão o
    -- histórico cresce para sempre dentro de um payload que é lido a cada abertura.
    'avulsas', coalesce((
      select jsonb_agg(jsonb_build_object(
        'id', t.id, 'title', t.title, 'area', t.area, 'area_icon', t.area_icon,
        'description', t.description, 'deadline', t.deadline, 'status', t.status,
